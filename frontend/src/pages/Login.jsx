...
export default function Login({ onNavigate }) {
  ...
  const updateField = ({ target }) => { const { name, value, checked, type } = target; setForm((v) => ({ ...v, [name]: type === 'checkbox' ? checked : value })); setErrors((v) => ({ ...v, [name]: '' })) }
  const submit = (event) => { event.preventDefault(); const next = {}; if (!form.email.trim()) next.email = 'Email address is required.'; else if (!emailPattern.test(form.email)) next.email = 'Enter a valid email address.'; if (!form.password) next.password = 'Password is required.'; else if (form.password.length < 6) next.password = 'Password must be at least 6 characters.'; setErrors(next); if (!Object.keys(next).length) console.info('EasyPark sign-in form validated', { email: form.email, remember: form.remember }) }
  ...
  return <main className="grid min-h-screen bg-slate-50 font-sans lg:grid-cols-[.94fr_1.06fr]">
    <section className="mx-auto flex min-h-screen w-[min(100%-42px,460px)] flex-col justify-center py-8 lg:w-[min(100%-80px,460px)]">
      ...
      <div className="relative mt-9 rounded-[18px] border border-slate-200 bg-white px-6 py-8 shadow-[0_18px_42px_rgba(15,23,42,.055)] sm:px-[34px] before:absolute before:top-0 before:left-8 before:h-[3px] before:w-12 before:rounded-b-lg before:bg-green-500 before:content-['']">
        ...
        <form className="grid gap-5" noValidate onSubmit={submit}>
          <label className="grid gap-2 text-[12px] font-semibold text-slate-900"><span>Email address</span><input className={input} name="email" type="email" autoComplete="email" placeholder="you@example.com" value={form.email} onChange={updateField} aria-invalid={Boolean(errors.email)} />{errors.email && <small className="text-[10px] font-medium text-red-600">{errors.email}</small>}</label>
          <label className="grid gap-2 text-[12px] font-semibold text-slate-900"><span>Password</span><span className="relative"><input className={`${input} pr-12`} name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Enter your password" value={form.password} onChange={updateField} aria-invalid={Boolean(errors.password)} /><button className="absolute top-0 right-0 grid h-12 w-12 place-items-center text-slate-500 hover:text-green-500" type="button" onClick={() => setShowPassword((v) => !v)} aria-label="Toggle password visibility"><Icon className="h-[18px] w-[18px]"><path d="M2.3 10.9C3.2 9 7 4 12 4s8.8 5 9.7 6.9a1.9 1.9 0 0 1 0 1.8C20.8 13.6 17 18 12 18s-8.8-4.4-9.7-6.3a1.9 1.9 0 0 1 0-.8Z" /><circle cx="12" cy="11" r="3" /></Icon></button></span>{errors.password && <small className="text-[10px] font-medium text-red-600">{errors.password}</small>}</label>
          <div className="flex items-center justify-between text-[11px] text-slate-700"><label className="inline-flex items-center gap-2"><input className="h-4 w-4 accent-green-500" name="remember" type="checkbox" checked={form.remember} onChange={updateField} />Remember me</label><a className="font-semibold text-green-600 hover:underline" href="#forgot-password">Forgot password?</a></div>
    ...
    <aside className="relative hidden place-items-center overflow-hidden bg-[linear-gradient(145deg,#0f172a,#123a2a_48%,#15803d)] p-12 text-white lg:grid">
      ...
      <div className="relative z-10 text-center"><div className="mx-auto grid h-48 w-48 place-items-center rounded-full border-[14px] border-green-400 bg-white/10 shadow-[0_0_80px_rgba(34,197,94,.25)]"><span className="grid h-24 w-24 place-items-center rounded-[30px_30px_30px_0] bg-white text-4xl font-bold text-green-600">P</span></div><h2 className="mt-12 text-4xl font-bold tracking-tight">Your space is<br />waiting for you.</h2><p className="mx-auto mt-4 max-w-xs text-[13px] leading-6 text-white/70">Find, reserve, and manage your parking in just a few taps.</p><div className="mt-6 flex justify-center gap-5 text-[10px]"><span className="inline-flex items-center gap-1"><CheckIcon />Instant booking</span><span className="inline-flex items-center gap-1"><CheckIcon />Best locations</span></div></div>
    </aside>
  </main>
}
