...
export default function Login({ onNavigate }) {
  ...
  const requestVerificationCode = async (event) => {
    ...
    if (!resetForm.identifier.trim())
      next.identifier = "Email or telephone number is required.";
    setResetErrors(next);
    if (Object.keys(next).length) {
      setResetMessage({
        type: "error",
        text: "Please correct the highlighted field.",
      });
      return;
    }
    ...
    try {
      ...
      setResetMessage({
        type: "success",
        text: "Your request was sent to an administrator. The OTP will be delivered manually.",
      });
    } catch (error) {
  ...
  const completeReset = async (event) => {
    ...
    try {
      ...
      setResetMessage({
        type: "success",
        text: "Your password has been reset. You can now sign in with it.",
      });
    } catch (error) {
      setResetMessage({ type: "error", text: error.message });
      setIsResetLoading(false);
      ...
      <main className="flex-1 grid lg:grid-cols-[.94fr_1.06fr]">
        ...
        <section className="mx-auto flex w-[min(100%-42px,460px)] flex-col justify-center py-8 lg:w-[min(100%-80px,460px)]">
          <button
            ...
            className="auth-back-link inline-flex w-fit items-center gap-2 text-sm font-semibold text-[#475569] transition hover:text-[#16A34A]"
          >
            <Icon className="h-4 w-4">
          ...
          <div className="auth-brand-logo-log-reg mt-7 flex items-center gap-3">
            <img
              src="/Logo_icon.png"
              alt="EasyPark"
          ...
          <div className="relative mt-9 rounded-[18px] border border-slate-200 bg-white px-6 py-8 shadow-[0_18px_42px_rgba(15,23,42,.055)] sm:px-[34px] before:absolute before:top-0 before:left-8 before:h-[3px] before:w-12 before:rounded-b-lg before:bg-green-500 before:content-['']">
            {authView === "login" ? (
              <>
                ...
                {serverError && (
                  <div
                    className="mb-1 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[11px] leading-5 text-red-700"
                    role="alert"
                  >
                    {serverError}
                  </div>
              ...
              <div>
                <div className="mb-7">
                  ...
                  <p className="mt-2 text-[13px] leading-6 text-slate-700">
                    {resetStep === "request"
                      ? "Enter your email or phone. An administrator will send the OTP manually."
                      : resetStep === "reset"
                        ? "Enter the OTP sent by the administrator and choose a new password."
                        : "Your EasyPark account is ready to use."}
                  </p>
                </div>

                {resetMessage && (
                  <div
                    className={`mb-5 rounded-xl border px-4 py-3 text-[11px] leading-5 ${resetMessage.type === "error" ? "border-red-200 bg-red-50 text-red-700" : "border-green-200 bg-green-50 text-green-700"}`}
                  ...
                  >
                    <label className="grid gap-2 text-[12px] font-semibold text-slate-900">
                      ...
                      <input
                        ...
                        aria-invalid={Boolean(resetErrors.identifier)}
                      />
                ...
                {resetStep === "reset" && (
                  ...
                  >
                    <label className="grid gap-2 text-[12px] font-semibold text-slate-900">
                      ...
                      <small className="font-normal text-[10px] text-slate-500">
                        Ask the administrator to send the OTP to you.
                      </small>
                    </label>
                    <label className="grid gap-2 text-[12px] font-semibold text-slate-900">
                    ...
                    <button
                      type="button"
                      className="text-center text-[11px] font-semibold text-green-600 hover:underline"
                      onClick={() => {
                        setResetStep("request");
                        setResetMessage(null);
                      }}
                ...
                {resetStep === "success" && (
                  <button
                    ...
                    className="flex h-[52px] w-full items-center justify-between rounded-xl bg-green-500 px-5 text-[13px] font-semibold text-white shadow-[0_10px_20px_rgba(34,197,94,.22)] transition hover:-translate-y-px hover:bg-green-600"
                  >
                    Back to sign in
                    <Icon className="h-[18px] w-[18px]">
                      <path d="M5 12h13M13 6l6 6-6 6" />
                ...
                {resetStep !== "success" && (
                  <button
                    type="button"
                    onClick={returnToLogin}
                    className="mt-6 w-full text-center text-[11px] font-semibold text-green-600 hover:underline"
                  >
                    Back to sign in
                  </button>
                )}
        ...
        <aside className="auth-visual-panel relative hidden min-h-screen overflow-hidden bg-[#0F172A] lg:block">
          <img
            src="/Login_img.png"
            alt="EasyPark parking experience"
            className="absolute inset-0 h-full w-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A]/90 via-[#0F172A]/25 to-[#0F172A]/25" />
          <div className="auth-visual-content absolute inset-x-12 bottom-12 text-white">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-2 text-xs font-semibold backdrop-blur">
              <CheckIcon /> Smarter parking
            </span>
            ...
            <p className="auth-visual-copy mt-3 max-w-sm text-sm leading-6 text-white/80">
              Find, reserve, track, and manage your parking in just a few taps.
            </p>
          </div>
        </aside>
      </main>
